...
! ***********************************************************************
      ...
      subroutine data_for_extra_history_columns(id, id_extra, n, names, vals, ierr)
         ...
         integer :: i
         integer :: i_bif
         real(dp) :: bind_g,bind_b,bind_h
         real(dp) :: E_bindg,E_bindb,E_bindh
         real(dp) :: core_mass,ce_core_radius
         type (star_info), pointer :: s
         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /=0) return
            ...
            i_bif = 1
            do i =1, s% nz, 1
                if (s% X(i) .ge. 0.1) then
                i_bif = i
                end if
                !write (*,*) "cwang", s% nz, i_bif,s% m(i_bif) 
            end do
            core_mass = s% m(i_bif)
            ce_core_radius = s% r(i_bif)
            !write(*,*) "cwang", s%nz, i_bif, core_mass
            bind_g = 0d0
            bind_b = 0d0
            bind_h = 0d0
            do i = i_bif, 1, -1
                if (i < i_bif) then
                bind_g = bind_g-6.67D-8*s% m(i)*s% dm(i)/s% r(i)
                bind_b = bind_b+s% energy(i)*s% dm(i)
                bind_h = bind_h+s% Pgas(i)*s% dm(i)/s% rho(i)
               ! write(*,*) "cwang1", bind_g, s% m(i), s% dm(i), s% r(i)
                end if
                !write(*,*) "cwang2", 6.67D-8*s% m(i)*s% dm(i)/s% r(i)
            end do
   
            !column 1
            names(1) = "ce_core_mass"
            vals(1) = s% m(i_bif)/1.9892d33
            ...
            !columns 3
            names(3) = "bind_b"
            vals(3) = bind_b+bind_g
 
            !columns 4
            names(4) = "bind_h"
            vals(4) = bind_h+bind_g+bind_b
 
            !columns 5
            names(5) = "lambda_g"
            vals(5) = - (6.67D-8*s% m(1) *(s% m(1)-s% m(i_bif)))/(bind_g*s% photosphere_r*6.9598d10)

            !columns 6
            names(6) = "lambda_b"
            vals(6) = - (6.67D-8*s% m(1) *(s% m(1)-s% m(i_bif)))/((bind_g+bind_b)*s% photosphere_r*6.9598d10)

            !columns 7
            names(7) = "lambda_h"
            vals(7) = - (6.67D-8*s% m(1) *(s% m(1)-s% m(i_bif)))/((bind_g+bind_b+bind_h)*s% photosphere_r*6.9598d10)
            !columns 8
            names(8) = "ce_core_radius"
            vals(8) = s% r(i_bif)/6.957d11
         !cwang CEE end
         ierr = 0
         !change by cwang
         !write (*,*) n
      ...
      subroutine data_for_extra_profile_columns(id, id_extra, n, nz, names, vals, ierr)
         ...
         names(5) = 'P/rho'
         do k = 1, nz
         vals(k,1) = -6.67D-8*s% m(k) * s% dm(k) / s% r(k)
         vals(k,2) = s% energy(k) * s% dm(k)
         vals(k,3) = s% Pgas(k) *s% dm(k) / s% rho(k)
         vals(k,4) = vals(k,1)+vals(k,2)+vals(k,3)
         vals(k,5) = s% Pgas(k) / s% rho(k)
         end do
